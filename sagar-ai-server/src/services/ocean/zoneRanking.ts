...
function chlorophyllScore(value?: number): {
  score: number;
  reason?: string;
} {
...
export function rankFishingZones(
  ...
  return candidates
    .map((zone) => {
      ...
      reasons.push(
        `Configured suitability: ${zone.suitability ?? "unknown"}`
      );
      ...
      if (centroid) {
        ...
        const restricted = geofences.find(
          (result) => result.type !== "navigation"
        );
        ...
        if (restricted) {
          ...
          reasons.push(
            restricted.inside
              ...
              : `${restricted.boundaryName} is approximately ${restricted.distanceKm.toFixed(
                  1
                )} km away`
          );
        }
...
export function getZonesToAvoid(
  options: { query?: string } = {}
): RankedFishingZone[] {
  return rankFishingZones(options).filter(
    (zone) => zone.recommendation === "AVOID"
  );
}
